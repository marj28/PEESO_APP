        ...
        <v-card>
          <v-card-text>
            <v-row>
              ...
              >
                <img
                  :src="$store.state.img_dir + 'program.png'"
                  width="48"
                >
              </v-col>
              <v-col cols="8">
        ...
        >
          <v-card-text>
            <v-row>
              ...
              >
                <img
                  :src="$store.state.img_dir + 'job.png'"
                  width="48"
                >
              </v-col>
              <v-col cols="8">
    ...
    <v-row>
      <v-col cols="8">
        <v-card>
          ...
          <v-card-text>
              <v-row>
                  <v-col cols="4">
                    <va-file-upload :preset="company.medias.logo" @UploadEvent="imageUrl"/>
                  </v-col> 
                  <v-col cols="8">
                    <v-text-field 
      ...
      <v-col cols="4">
        ...
        "
            >YOUR JOB POSTS</span>
            <v-spacer/>
            <v-btn x-small color="info"  @click="$router.push('/job-post').catch(err => {})">See all...</v-btn>
          </v-toolbar>
          <v-card-text>
            <v-list three-line>
              <template v-for="(item, index) in jobs">
                ...
                >
                  ...
                  <v-list-item-content>
                    <v-list-item-title><a  @click="$router.push('post/' + item.id).catch(err => {})">{{item.title}}</a> - <span class="text-caption">{{item.status}}</span></v-list-item-title>
                    <v-list-item-subtitle> <em class="text-info">{{$moment(item.created_dt).startOf('hour').fromNow()}}</em> - {{item.post_meta.company}} - {{item.post_meta.company_address}} </v-list-item-subtitle>
                  </v-list-item-content>
                </v-list-item>
              </template>
            </v-list>
