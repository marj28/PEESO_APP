  ...
  <div>
        ...
        <v-col cols="6">
          ...
          >
            <v-card-text class="text-center">
              <span><v-icon left>mdi-account-hard-hat-outline</v-icon>As Jobseeker</span> 
            </v-card-text>   
          </v-card>
        </v-col>
        <v-col cols="6">
    ...
    <v-row>
      <v-col class="pt-4 text-center">
        <span>Already have an account</span> <v-btn
          text
          color="warning"
          @click="$router.push('/login').catch(err => {})"
        >
          Login here <v-icon right>
    ...
    methods: {
      ...
      userReg() {
        ...
        this.loading = true
          ...
          this.$http.post('user/register', this.account).then(response => {
